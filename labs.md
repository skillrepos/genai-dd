...
1. In our repository, we have a simple neural net coded in Python. The file name is simple_nn.py. Open the file either by clicking on [**simple_nn.py**](./simple_nn.py) or by entering the command below in the codespace's terminal.
...
1. In our repository, we have a simple program that can load a model and print out tokens generated by it. The file name is simple_tokenizer.py. You can view the file either by clicking on [**simple_tokenizer.py**](./simple_tokenizer.py) or by entering the command below in the codespace's terminal.
...
4. Now let's run the simple_tokenizer program with the bert-base-uncased model. Enter the command below. This will download some of the files you saw on the *Files* tab.
```
...
```
python simple_vectors.py bert-base-cased 768
```
...
10. If you want, you can try running the *simple_vectors.py* program with a different model to see results from other models (such as we used in lab 2) and words that are very close like *embeddings*, *tokenization*, *subwords*.
